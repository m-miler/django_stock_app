...
class StockModelsTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.current_price = factories.StockPriceFactory(
            date="2023-01-18", company_abbreviation=self.company
        )
        self.last_price = factories.StockPriceFactory(
            date="2023-01-11", company_abbreviation=self.company
        )
    ...
    def test_create_new_record_of_stock_price_db(self):
        ...
        stock_price = StockPrices.objects.get(
            company_abbreviation=self.company.company_abbreviation, date=self.TODAY
        )
        self.assertEqual(
    ...
    def test_stock_price_get_weekly_price(self):
        ...
        self.assertEqual(
            float(close_last_week_price),
            round(
                (
                    (self.current_price.close_price - self.last_price.close_price)
