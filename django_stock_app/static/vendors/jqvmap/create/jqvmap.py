...
class JQVMap:
    ...
    def getJSCode(self):
        ...
        header = (
            "/** JQVMap " + self.projection["type"] + " map for " + self.name + "  */"
        )
        js = (
...
class Converter:
    def __init__(self, config):
        ...
        if (
            args.get("emulate_longitude0") is None
            and (self.projection == "merc" or self.projection == "mill")
    ...
    def convert(self, data_source, output_file):
        codes = map(
            lambda g: g.properties[self.config["code_field"]], data_source.geometries
        )
        main_codes = copy.copy(codes)
        self.map.insets = []
        envelope = []
        for inset in self.insets:
            insetBbox = self.renderMapInset(
                data_source, inset["codes"], inset["left"], inset["top"], inset["width"]
            )
            ...
            self.map.insets.append(
                {
                    "bbox": [
                        {"x": insetBbox[0], "y": -insetBbox[3]},
                        {"x": insetBbox[2], "y": -insetBbox[1]},
                    ],
                    "left": inset["left"],
                    "top": inset["top"],
                    "width": inset["width"],
        ...
        insetBbox = self.renderMapInset(
            data_source, main_codes, self.left, self.top, self.width
        )
        insetHeight = (insetBbox[3] - insetBbox[1]) * (
            self.width / (insetBbox[2] - insetBbox[0])
        )
        envelope.append(
            shapely.geometry.box(
                self.left, self.top, self.left + self.width, self.top + insetHeight
            )
        )
        ...
        self.map.projection = {
            ...
            "centralMeridian": float(self.longitude0),
        }
        ...
        if self.for_each is not None:
            for code in codes:
                ...
                for param in ("input_file", "output_file", "where", "name"):
                    childConfig[param] = childConfig[param].replace(
                        "{{code}}", code.lower()
                    )
    ...
    def renderMapInset(self, data_source, codes, left, top, width):
        envelope = []
        geometries = filter(
            lambda g: g.properties[self.config["code_field"]] in codes,
        ...
        for geometry in geometries:
            ...
            if self.simplify_tolerance:
                geom = geom.simplify(
                    self.simplify_tolerance * scale, preserve_topology=True
                )
                ...
                for ring in rings:
                    for pointIndex in range(len(ring.coords)):
                        ...
                        if pointIndex == 0:
                            path += "M" + str(
                                round(
                                    (point[0] - bbox[0]) / scale + left, self.precision
                                )
                            )
                            path += "," + str(
                                round(
                                    (bbox[3] - point[1]) / scale + top, self.precision
                                )
                            )
                        else:
                            path += "l" + str(
            ...
            self.map.addPath(
                path,
                geometry.properties[self.config["code_field"]].lower(),
                geometry.properties[self.config["name_field"]],
            )
        return bbox
...
class DataSource:
    def __init__(self, config):
        default_config = {"projection": "merc", "longitude0": 0}
        ...
        projString = (
            "+proj="
            + str(self.config["projection"])
            + " +a=6381372 +b=6381372 +lat_0=0"
        )
            ...
            if geometry.geom is not None:
                ...
                for index, field in enumerate(self.fields):
                    if field["name"] in geometry.properties:
                        feature.SetField(
                            index, geometry.properties[field["name"]].encode("utf-8")
                        )
                    else:
                        feature.SetField(index, "")
                feature.SetGeometryDirectly(
                    ogr.CreateGeometryFromWkb(shapely.wkb.dumps(geometry.geom))
                )
...
class PolygonSimplifier:
    ...
    def simplifyRing(self, ring):
        ...
        while not isPivot and pointIndex < len(coords):
            ...
            pointIndex += 1
            isPivot = (len(self.connections[pointStr]) > 2) or (
                pointStr in self.pivotPoints
            )
        ...
        if not isPivot:
            ...
            if len(simpleRing.coords) <= 2:
                ...
                simpleLineKey = (
                    self.format % coords[0]
                    + ":"
                    + self.format % coords[1]
                    + ":"
                    + self.format % coords[-1]
                )
            ...
            for i in range(1, len(points)):
                ...
                if (len(self.connections[pointStr]) > 2) or (
                    pointStr in self.pivotPoints
                ):
...
class Processor:
    ...
    def merge(self, config, data_source):
        ...
        for rule in config["rules"]:
            ...
            geometries = filter(
                lambda g: expression(g.properties), data_source.geometries
            )
            geometries = map(lambda g: g.geom, geometries)
            new_geometries.append(
                Geometry(shapely.ops.cascaded_union(geometries), rule["fields"])
            )
    ...
    def join_data(self, config, data_source):
        ...
        data_source.fields = data_source.fields + filter(
            lambda f: f["name"] not in field_names, config["fields"]
        )

    def remove(self, config, data_source):
        expression = data_source.parse_manager.parse(config["where"])
        data_source.geometries = filter(
            lambda g: not expression(g.properties), data_source.geometries
        )

    def remove_fields(self, config, data_source):
        data_source.fields = filter(
            lambda f: f.name not in config["fields"], data_source.fields
        )

    def remove_other_fields(self, config, data_source):
        data_source.fields = filter(
            lambda f: f["name"] in config["fields"], data_source.fields
        )

    def buffer(self, config, data_source):
        for geometry in data_source.geometries:
            geometry.geom = geometry.geom.buffer(
                config["distance"], config["resolution"]
            )

    def simplify_adjancent_polygons(self, config, data_source):
        simple_geometries = PolygonSimplifier(
            map(lambda g: g.geom, data_source.geometries)
        ).simplify()
    ...
    def intersect_rect(self, config, data_source):
        transform = osr.CoordinateTransformation(
            data_source.layer.GetSpatialRef(), data_source.spatialRef
        )
