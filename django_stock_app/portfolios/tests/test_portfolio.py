...
class PortfolioTest(TestCase):
    def setUp(self) -> None:
        ...
        self.portfolio_stock = factories.PortfolioStockFactory(portfolio_id=self.portfolios[1],
    ...
    def test_get_portfolio_detailed_path(self):
        ...
        response = self.client.get(reverse('portfolio-detail', kwargs={'portfolio': 'PortfolioTest0'}))
    ...
    def test_create_portfolio_success(self):
        ...
        self.client.force_login(self.user)
        response = self.client.post(reverse('portfolio-create'), data={'name': 'PortfolioTest3'})
        self.assertRedirects(response, expected_url=reverse('portfolio-detail', kwargs={'portfolio': 'PortfolioTest3'}),
                             target_status_code=HTTPStatus.FOUND, fetch_redirect_response=False)
    ...
    def test_post_delete_portfolio(self):
        ...
        request.user = self.user
        response = DeletePortfolio.as_view()(request, portfolio='PortfolioTest1')
        self.assertRedirects(response, expected_url=reverse('home'), target_status_code=HTTPStatus.FOUND,
    ...
    def test_buy_stocks(self):
        ...
        self.client.force_login(self.user)
        for num in range(2):
            request = self.client.post(reverse('portfolio-buy', kwargs={'portfolio': 'PortfolioTest0'}),
                                                data={
                                                    'stock': self.company.company_abbreviation,
        ...
        stock = PortfolioStocks.objects.filter(portfolio_id='TestUser0_PortfolioTest0').first()
        self.assertEqual(stock.amount, 4)
        self.assertEqual(float(stock.portfolio_id.balance), 10000 - 4 * self.current_price.close_price)

    def test_buy_stock_view(self):
        """ Test the buy stock view."""
        self.client.force_login(self.user)
        response = self.client.get(reverse('portfolio-buy', kwargs={'portfolio': 'PortfolioTest0'}))

        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertTemplateUsed(response, template_name='portfolios/buy_stock_portfolio.html')
    ...
    def test_buy_stocks_form_validators(self):
        ...
        form = BuyStockPortfolioForm(data={
            'stock': self.company.company_abbreviation,
            'full_stock_name': self.company.company_full_name,
            'stock_price': self.current_price.close_price,
            'amount': 1000,
        ...
        self.assertEqual(form.errors['value'], ['The amount is higher then portfolio balance!'])

    def test_sell_stock(self):
        """ Test if stock sales are correct."""
        self.client.force_login(self.user)
        response = self.client.post(reverse('portfolio-sell', kwargs={'portfolio': 'PortfolioTest1'}),
                                    data={
                                        'stock': self.company.company_abbreviation,
                                        'full_stock_name': self.company.company_full_name,
                                        'stock_price': self.current_price.close_price,
        ...
        stocks = PortfolioStocks.objects.filter(portfolio_id='TestUser0_PortfolioTest1').first()
    ...
    def test_sell_stock_form_validation(self):
        ...
        form = SellStockPortfolioForm(data={
            'stock': self.company.company_abbreviation,
            'full_stock_name': self.company.company_full_name,
            'stock_price': self.current_price.close_price,
            'amount': 20,
