...
class SellStockPortfolioForm(forms.ModelForm):
    ...
    def __init__(self, portfolio_id=None, *args, **kwargs):
        ...
        stock_choice = PortfolioStocks.objects.filter(portfolio_id=portfolio_id).only(
            "stock"
        )
        self.fields["stock"] = forms.ModelChoiceField(
            queryset=stock_choice, to_field_name="stock"
        )
        self.fields["stock"].label = "Stock Ticker"
        self.fields["stock"].help_text = ""
        self.fields["stock"].widget.attrs.update(
            {"class": "form-control", "placeholder": "Stock Name"}
        )
        ...
        self.fields["amount"].widget.attrs.update({"class": "form-control", "min": 0})
    ...
    def clean_amount(self):
        ...
        current_amount_of_stock = (
            PortfolioStocks.objects.filter(portfolio_id=self.portfolio_id, stock=stock)
            .first()
            .amount
        )
