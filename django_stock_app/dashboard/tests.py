...
class DashboardTests(TestCase):
    ...
    def test_home_page_redirect(self):
        ...
        response_1 = self.client.get(reverse('dashboard', kwargs={'ticker': 'WIG20'}), follow=True)
        self.assertEqual(response.redirect_chain, [('/login/', HTTPStatus.FOUND)])
        self.assertEqual(response_1.redirect_chain, [('/login/?next=/dashboard/WIG20', HTTPStatus.FOUND)])
    ...
    def test_anonymous_user_redirect_to_login_page(self):
        ...
        ticker = 'WIG20'
        response = self.client.get(reverse('dashboard', kwargs={'ticker': ticker}), follow=True)
    ...
    def test_dashboard_view_contex(self):
        ...
        request = self.request_factory.get(reverse('dashboard', kwargs={'ticker': ticker}))
